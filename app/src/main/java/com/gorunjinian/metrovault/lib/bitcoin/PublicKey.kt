...
data class PublicKey(@JvmField val value: ByteVector) {
     ...
     * A public key is valid if it represents a point on the secp256k1 curve.
     */
    fun isValid(): Boolean = Crypto.isPubKeyValid(value.toByteArray())

    fun isEven(): Boolean = (value[0] == 2.toByte())

    fun isOdd(): Boolean = !isEven()

    fun xOnly(): XonlyPublicKey = XonlyPublicKey(this)
    ...
    operator fun minus(that: PublicKey): PublicKey = plus(-that)
     ...
     */
    fun toUncompressedBin(): ByteArray = Secp256k1.pubkeyParse(value.toByteArray())
     ...
     */
    fun p2pkhAddress(chainHash: BlockHash): String = when (chainHash) {
        Block.Testnet4GenesisBlock.hash, Block.Testnet3GenesisBlock.hash, Block.RegtestGenesisBlock.hash, Block.SignetGenesisBlock.hash -> Base58Check.encode(
            Base58.Prefix.PubkeyAddressTestnet, hash160())
     ...
     */
    fun p2wpkhAddress(chainHash: BlockHash): String {
     ...
     */
    fun p2trAddress(chainHash: BlockHash): String = xOnly().p2trAddress(chainHash)
    ...
    companion object {
        @JvmField
        val Generator: PublicKey = parse(Hex.decode("0479be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798483ada7726a3c4655da4fbfc0e1108a8fd17b448a68554199c47d08ffb10d4b8"))

        @JvmStatic
        fun parse(pub: ByteArray): PublicKey = PublicKey(compress(pub))

        @JvmStatic
        fun compress(pub: ByteArray): ByteArray = when {
        ...
        @JvmStatic
        fun fromHex(hex: String): PublicKey = PublicKey(Hex.decode(hex))
    }
}