...
object Script {
    ...
    const val TAPROOT_LEAF_MASK: Int = 0xfe
    const val TAPROOT_LEAF_TAPSCRIPT: Int = 0xc0
    ...
    val False: ByteVector = ByteVector.empty
    ...
    fun scriptIterator(script: ByteArray): Iterator<ScriptElt> = scriptIterator(ByteArrayInput(script))
     ...
     */
    @JvmStatic
    fun parse(input: Input): List<ScriptElt> = scriptIterator(input).asSequence().toList()

    @JvmStatic
    fun parse(blob: ByteArray): List<ScriptElt> = parse(ByteArrayInput(blob))
    ...
    @JvmStatic
    fun isUpgradableNop(op: ScriptElt): Boolean = when (op) {
        OP_NOP1, OP_NOP4, OP_NOP5, OP_NOP6, OP_NOP7, OP_NOP8, OP_NOP9, OP_NOP10 -> true
        else -> false
    }

    @JvmStatic
    fun isSimpleValue(op: ScriptElt): Boolean = when (op) {
    ...
    @JvmStatic
    fun isDisabled(op: ScriptElt): Boolean = when (op) {
    ...
    @JvmStatic
    fun encodeNumber(value: Int): ByteVector = encodeNumber(value.toLong())
    ...
    @JvmStatic
    fun isNativeWitnessScript(script: List<ScriptElt>): Boolean = when {
        script.size != 2 -> false
    ...
    @JvmField
    val pay2anchor: List<ScriptElt> = listOf(OP_1, OP_PUSHDATA(ByteVector("4e73")))

    /** An empty witness script is used to spend [pay2anchor] outputs. */
    @JvmField
    val witnessPay2anchor: ScriptWitness = ScriptWitness.empty
    ...
    fun sigHashType(sig: ByteArray): Int = when (sig.size) {
        64 -> SigHash.SIGHASH_DEFAULT
        65 if sig[64].toInt() == SigHash.SIGHASH_DEFAULT -> error("invalid sig hashtype")
