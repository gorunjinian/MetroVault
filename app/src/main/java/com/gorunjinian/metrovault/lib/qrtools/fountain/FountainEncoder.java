...
public class FountainEncoder {
    ...
    private byte[] mix(List<Integer> partIndexes) {
        return partIndexes.stream().reduce(new byte[fragmentLen], (result, index) -> xor(fragments.get(index), result),
                FountainEncoder::xor);
    }
    ...
    public record Part(long seqNum, int seqLen, int messageLen, long checksum, byte[] data) {
        ...
        public byte[] toCborBytes() {
            ...
            public static Part fromCborBytes(byte[] cborData) throws CborException {
                ...
                return new Part(seqNum.getValue().longValue(), seqLen.getValue().intValue(),
                        messageLen.getValue().intValue(), checksum.getValue().longValue(), data.getBytes());
            }
            ...
            @Override
            public int hashCode() {
                int result = Long.hashCode(seqNum);
