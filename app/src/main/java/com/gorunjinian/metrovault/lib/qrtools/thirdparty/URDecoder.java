...
public class URDecoder {
    ...
    public boolean receivePart(String string) {
        try {
            ...
            if (matcher.matches()) {
                int seqNum = Integer.parseInt(Objects.requireNonNull(matcher.group(1)));
                int seqLen = Integer.parseInt(Objects.requireNonNull(matcher.group(2)));
                ...
                if (fountainDecoder.getResult() == null) {
                    ...
                    result = new Result(ResultType.SUCCESS,
                            new UR(parsedURString.type, fountainDecoder.getResult().data), null);
