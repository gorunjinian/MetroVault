package com.gorunjinian.metrovault.lib.qrtools.fountain;
...
import static com.gorunjinian.metrovault.lib.qrtools.fountain.FountainUtils.chooseFragments;
...
public class FountainDecoder {
    ...
    private static class Part {
        ...
        Part(FountainEncoder.Part part) {
            this.partIndexes = chooseFragments(part.seqNum(), part.seqLen(), part.checksum());
            this.data = part.data();
        }
    ...
    private void printPartEnd() {
        ...
        System.out.println("processed: " + processedPartsCount + " expected: " + getExpectedPartCount() + " received: "
                + recievedPartIndexes.size() + " percent: " + percent + "%");
    }
    ...
    private void printState() {
        ...
        System.out.println("parts: " + getExpectedPartCount() + ", received: " + sortedReceived + ", mixed: " + mixed
                + ", queued: " + queuedParts.size() + ", result: " + result);
    }
    ...
    private void reduceMixed(Part by) {
        ...
        List<Part> reducedParts = mixedParts.values().stream().map(part -> reducePart(part, by))
                .collect(Collectors.toList());
    ...
    private Part reducePart(Part a, Part b) {
        // If the fragments mixed into `b` are a strict (proper) subset of those in
        // `a`...
    ...
    private void processSimplePart(Part part) {
        ...
        if (recievedPartIndexes.equals(expectedPartIndexes)) {
            ...
            List<Part> sortedParts = simpleParts.values().stream().sorted(Comparator.comparingInt(Part::getIndex))
                    .collect(Collectors.toList());
    ...
    private boolean validatePart(FountainEncoder.Part part) {
        ...
        if (expectedPartIndexes == null) {
            // Record the things that all the other parts we see will have to match to be
            // valid.
            expectedPartIndexes = IntStream.range(0, part.seqLen()).boxed().collect(Collectors.toSet());
            expectedMessageLen = part.messageLen();
            expectedChecksum = part.checksum();
            expectedFragmentLen = part.data().length;

            return true;
        } else {
            return getExpectedPartCount() == part.seqLen() && expectedMessageLen == part.messageLen()
                    && expectedChecksum == part.checksum() && expectedFragmentLen == part.data().length;
        }
    }
